...
/**
 * SectionInitial
 * Muestra la primera sección del flujo de caja con las
 * subsecciones BANCOS, CLIENTES y EOR en pestañas.
 */
        ...
        <table className="viewer-table">
          <thead>
            <tr>
              ...
              <th className="col-right" style={{ width: '150px' }}>Valor F</th>
              <th className="col-right" style={{ width: '150px' }}>Valor G</th>
          ...
          <tbody>
            {filteredRows.length === 0 && tabInsertions.length === 0 ? (
              <tr>
                <td colSpan={6} className="td-empty">Sin resultados para este filtro</td>
              </tr>
            ) : (
              filteredRows.map((row, i) => (
                <tr key={i} className={i % 2 === 0 ? 'tr-even' : ''}>
                  ...
                  <EditableCell rowIdx={row._row} colIdx={5} raw={row.valorF} edits={edits} onCellEdit={onCellEdit} format="cop"  className="col-right td-valor" />
                  <EditableCell rowIdx={row._row} colIdx={6} raw={row.valorG} edits={edits} onCellEdit={onCellEdit} format="cop"  className="col-right td-valor" />
            ...
            <tr className="tr-add-row">
              <td colSpan={6}>
                <button
                  ...
                  title={current.rows.length === 0 ? 'Esta sección no tiene filas aún' : ''}
                  onClick={() => {
